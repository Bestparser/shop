...
class Shop_MultKeysController extends Lv7CMS_Controller_Backend
{
	...
	{
        parent::preDispatch();
		if (!$this->_acl->access('shopMultKeys')) {
			return;
		}
	...
	public function postDispatch()
	{
		...
		$this->view->headScript()->appendFile($this->_dataUrl . '/js/modules/shop/multKeys.js');
	}
	...
	public function formAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			if ($form->isValid($this->getRequest()->getPost())) {
				...
				$obj->keys = $form->getValue('keys');
				$obj->mult = $form->getValue('mult');
				$obj->activity = $form->getValue('activity');
				$this->_mapper->save($obj);
		
				$this->_helper->redirector->gotoRouteAndExit(array(), 'shop::multKeys');
			}
		} else {
			if (!empty($obj)) {
				$form->setDefault('name', $obj->name);
				$form->setDefault('keys', $obj->keys);
				$form->setDefault('mult', $obj->mult);
	...
	public function dataAction()
	{
		...
		$searchColumns = array('name', 'keys', 'mult');
		$orderColumns = array('activity', 'name', 'keys', 'mult');
		...
		if (is_array($data)) {
			...
			foreach ($data as $item) {
				...
				$row[] = $item->mult;
				$row[] = '<a href="' . $this->view->url($params, 'shop::multKeysForm') .'"
							class="button-edit">' . $this->view->translate('изменить') . '</a>
							<a href="'. $this->view->url($params, 'shop::multKeysDelete') . '" 
